...
function show() {
    ...
    console.log('page '+page+" / "+numPages)
...
function activePlayer(player) {
    ...
    playinfo("C'est au tour du joueur "+player+" de jouer !")
}
...
function gameOver() {
    ...
    setTimeout(() => {
        ...
        if(score1 > score2) {
            playinfo("Le joueur 1 a gagné !")
        }
        else if(score1 < score2) {
            playinfo("Le joueur 2 a gagné !")
        } else {
            playinfo("Les deux joueurs sont vainqueurs !")
        }
        setTimeout(() => {
            document.getElementById("play-info").innerHTML += "<br><br><button onclick='launchGame()' class='game-button'>Rejouer</button>"
        }, 3000)
    }, 3000)
}
